...
% option 1: using the sum of absoulute value of correlations of each neuron with all
% other neurons as a basic measure of neuron centrality
NeuronCent = -1 + sum(abs(CorrMat)) ;

% option 2: using the node centrality 
CentTh =median(reshape(CorrMat,1,N^2));
% NeuronCent = 20*NodeCentrality(CorrMat, CentTh, 0);
...
for i=1:N
    if (flag ~= 1) 
        %option 1 - bubble size according to centrality:
        %scatter(VPhysCol(i),VPhysRow(i),600*(0.8+0.4*NeuronCent(i)),'MarkerFaceColor',[clr(i,1) clr(i,2) clr(i,3)],'MarkerEdgeColor',[0 0 0 ]);
        % option 2 - bubble size constant:
        scatter(VPhysCol(i),VPhysRow(i),500,'MarkerFaceColor',[clr(i,1) clr(i,2) clr(i,3)],'MarkerEdgeColor',[0 0 0 ]);
        hold on;
    else
