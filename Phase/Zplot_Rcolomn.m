...
function Zplot_Rcolomn(Rcolomn, ref)
clc; home;
% close all;
...
if isscalar(ref) % if we choose to use an internal order of electrodes
...
else  % ref is not a scalar, we want to use an extrnal vector for ploting
    if (isnumeric(ref)==0)
        error('^^^ Extrenal forceVplot should be a vector to plot the PS by ')
    end
    if  (isnumeric(ref)==1 && length(ref)~=a)  % varargin is vector but not in the same length as PS
        error('^^^ Input vector length does not match PS dimentions - length should be the same')
    end
    if  (isvector(ref)==1 && length(ref)==a)
        ...
        for i = 1:n
            ...
            b=find(T~=1);
            m(i)=max(T(b)); %keep the maximum value of each matrix into m
            Vplot(i,:)=ref; %all matices are according to the order of PS #ref
        end
    end
end
